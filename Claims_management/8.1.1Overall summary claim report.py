...
SUM(COALESCE(c.CS_NOT_APPROVED, 0)) AS cs_not_approved,
            ...
            -- sum(case when c.InitiativeCode in (select distinct c.initcode from stg.cs_combined c left join stg.masters_auditex s on c.initcode = s.scheme_code where c.disbursement_count != s.Retailer_Apply_Count) as cs_not_approved,
            -- cast(0 as decimal(38,6)) as cs_not_approved,
            -- sum(case when (c.initiativeCode is not null and cs.initcode is null) and (c.initiativeCode is not null and dc.initcode is null) and 
            --(Amount_Disbursed_Monthly>0) then Amount_Disbursed_Monthly else 0 end) as cs_not_approved,
        ...
        sum(case when date_format(srn_shipdate,'yyyyMM')= {formatted_date} and 
                    --   date_format(shipdate,'yyyyMM')<> {formatted_date}
                      date_format(shipdate, 'yyyyMM') = date_format(add_months(to_date('{formatted_date}', 'yyyyMM'),-1),'yyyyMM')
                      and (trim(upper(matrix_check_monthly_release))=upper("Consider") and trim(upper(matrix_check_fortnight))=upper("Not Consider")) or (trim(upper(matrix_check_fortnight))=upper("Consider") and dayofmonth(shipdate) >14)
                      then Disallowance_SRN else 0 end
                      ) as sales_return_pm
        ...
        b.not_consider_for_calculation as Not_Consider_For_Calculation,
        a.cs_not_approved as CS_Not_Approved, 
        ...
        (a.wrong_rate+a.wrong_date+a.free_goods+a.wrong_channel+a.Disallowance_Settlement+b.sales_return_cm+b.sales_return_pm+a.cn_ar_adjustment+a.cn_srn+a.other_cn_disallowance+a.cash_claims+a.untagged_srn_recovery) as Total_Disallowance,
        (a.amountdisbursed_ihr-(a.wrong_rate+a.wrong_date+a.free_goods+a.Disallowance_Settlement+a.wrong_channel+b.sales_return_cm+b.sales_return_pm+a.cn_ar_adjustment+a.cn_srn+a.other_cn_disallowance+a.cash_claims+a.untagged_srn_recovery)) as Net_IHR_Disbursement
        from df_base a
...
# DBTITLE 1,claims_mgmt.overall_dist_summ_fortnight_v1 - original
# if end_date_value[-2:]=="14":
...
#             sum(case when c.InitiativeCode is not null 
#             and cs.initcode is null and dc.initcode is null and Amount_Disbursed_Fortnight > 0 then Amount_Disbursed_Fortnight else 0 end) as cs_not_approved,
...
#         (a.wrong_rate+a.wrong_date+a.free_goods+a.Disallowance_Settlement+a.wrong_channel+a.cn_ar_adjustment+a.cn_srn+a.other_cn_disallowance+a.cash_claims+a.untagged_srn_recovery+b.sales_return_cm) as Total_Disallowance,
#         (a.amountdisbursed_ihr-(a.wrong_rate+a.wrong_date+a.free_goods+a.Disallowance_Settlement+a.wrong_channel+a.cn_ar_adjustment+a.cn_srn+a.other_cn_disallowance+a.cash_claims+a.untagged_srn_recovery+b.sales_return_cm)) as Net_IHR_Disbursement
...
select 
    ...
    SUM(COALESCE(c.CS_NOT_APPROVED, 0)) AS cs_not_approved,
    0 as tsoi_adjustment,
    sum(case when (c.initiativeCode is not null and cs.initcode is not null) 
              or (c.initiativeCode is not null and m.scheme_Code is not null) 
              or (c.initiativeCode is not null and dc.initcode is not null) 
             then c.settelement_check_Amount end) as Disallowance_Settlement,

    sum(case when (c.initiativeCode is not null and cs.initcode is not null) 
              or (c.initiativeCode is not null and m.scheme_Code is not null) 
              or (c.initiativeCode is not null and dc.initcode is not null) 
             then c.Wrong_Rate end) as wrong_Rate,

    sum(case when (c.initiativeCode is not null and cs.initcode is not null) 
              or (c.initiativeCode is not null and m.scheme_Code is not null) 
              or (c.initiativeCode is not null and dc.initcode is not null) 
             then c.Wrong_date end) as wrong_date,

    sum(case when (c.initiativeCode is not null and cs.initcode is not null) 
              or (c.initiativeCode is not null and m.scheme_Code is not null) 
              or (c.initiativeCode is not null and dc.initcode is not null) 
             then c.free_goods end) as free_goods,

    sum(case when (c.initiativeCode is not null and cs.initcode is not null) 
              or (c.initiativeCode is not null and m.scheme_Code is not null) 
              or (c.initiativeCode is not null and dc.initcode is not null) 
             then c.Wrong_Channel end) as Wrong_Channel,
    0 as CN_AR_Adjustment,
    0 as CN_SRN,
    0 as Other_CN_Disallowance,
    0 as Cash_Claims,
