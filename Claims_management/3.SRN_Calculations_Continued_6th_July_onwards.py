...
# promotion_slab_check_aggregated \
#     .filter(
#         (F.col("RptMonthYear") == formatted_date) &
#         (F.col("ShipDate") >= F.lit(last_month_start).cast("date"))
#     ) \
#     .write.mode("append") \
#     .saveAsTable("claims_mgmt.srn_calcs")

# COMMAND ----------

# MAGIC %sql
# MAGIC DELETE FROM claims_mgmt.srn_calcs
# MAGIC WHERE invoice_number = return_invoice_number[0];
...
if str(formatted_date) == '202601':
  df = spark.sql("""
        SELECT SUM(disallowance) AS total_disallowance
        FROM claims_mgmt.srn_calcs
        WHERE RptMonthYear = '202601'
          AND (scheme_code IN ('LTR2601N00003550')
               OR scheme_code LIKE '%MRI')
    )
  df.display()
  LSS2602N3033_BC2_L1','LSS2602N3033_BC2_L3', 'LSS2602N3034_BC2_L1','LSS2602N3034_BC2_L3', 'LTR2602N00000011','LTR2602N00000012','LTT2602N00001067')
