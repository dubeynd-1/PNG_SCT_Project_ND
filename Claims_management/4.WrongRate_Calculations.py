...
# %sql
# select * from ihr_psr_combined where invoice_number = 'SCEMR-26-I003125'
...
Master_sdf = Master_sdf.filter(col("Scheme_Type").isNotNull() & col("Valid_From").isNotNull() & col("Valid_To").isNotNull())
Master_sdf.createOrReplaceTempView("Master_sdf")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from Master_sdf where scheme_code = 'LMP2606N4217'
...
Master_retailer = spark.table("Master_sdf")
# Master_retailer.display()
...
# MAGIC %sql
# MAGIC select * from intermediate where scheme_code = 'LMP2606N4217'
...
dedup_disc_calc as 
(
select * ,sum(gross_transact_amt) over(partition by scheme_code,invoice_number,Promotion_Group_Name ) as gross_transact_amt_sum_1, sum(item_unit_qty) over(partition By scheme_code,invoice_number,Promotion_Group_Name) as item_unit_qty_sum_1 , sum(net_transact_amt) over(partition by scheme_code,invoice_number,Promotion_Group_Name ) as net_transact_amt_sum_1, case when lower(Promotion_Get_Type)="discount every" then item_unit_qty*Promotion_Slab_Get_Discount else (gross_transact_amt*Promotion_Slab_Get_Discount)/100 end as amtdisbursed_calculated_at_sbf_lvl_1, 
case when lower(Promotion_Get_Type)="discount every" 
then item_unit_qty_sum_1*Promotion_Slab_Get_Discount else 
(gross_transact_amt_sum_1*Promotion_Slab_Get_Discount)/100 end as invcXscheme_lvl_Amount_disbursed_Calculated_1 from dedup
)
...
intermediate_modified1 = spark.sql('''
WITH exclude_samesbf AS (
    SELECT *,
           ROW_NUMBER() OVER (
               PARTITION BY invoice_number, scheme_code, SubbfName
               ORDER BY invcXscheme_lvl_Amount_disbursed_Calculated_1 DESC
           ) AS rnk
    FROM intermediate_modified
),

dedup1 AS (
    SELECT * EXCEPT(rnk)
    FROM exclude_samesbf
    WHERE rnk = 1
),
...
grouped AS (
    SELECT
        ...
        Promotion_Get_Type,
        Promotion_Slab_Get_Discount,
        retailer_code,

        SUM(gross_transact_amt) AS gross_transact_amt_agg,
        SUM(item_unit_qty) AS item_unit_qty_agg,
        SUM(net_transact_amt) AS net_transact_amt_agg,

        FIRST(amountdisbursed) AS amountdisbursed
...
SELECT *,
       CASE
           WHEN LOWER(Promotion_Get_Type) = 'discount every'
               THEN item_unit_qty_agg * Promotion_Slab_Get_Discount
           ELSE
               (gross_transact_amt_agg * Promotion_Slab_Get_Discount) / 100
       END AS invcXscheme_lvl_Amount_disbursed_Calculated
FROM grouped
''')
...
# --Amnt_disbursed_calculated1 shoulf be updated when both conditions are satisfied,
...
# %sql
# select * from slab_max_level_check where InitiativeCode = 'LMP2606N4217'
...
display(spark.sql(f"""select * from claims_mgmt.worng_rate_calcs 
